...
- [x] ✅ **伤害-施加侧(2026-07-02)**:`damage` actuator 动作——target `player`(→既有
  HealthSystem)或 NPC adjunctId(→ `utils/Combat.damageNpc` 单一伤害汇点);**仅 Game 模式**
  (权限矩阵一致)。**NPC 可死**:ba 行 slot 4 = hp(缺省无敌);死亡跑行为文档 `onDeath`
  动作(**掉落=spawn b5,零新原语,实测**)+ `npc.died` 事件;authored 死体**隐藏保留**
  (销毁会丢 draft 行——块重载复活),spawner 派生的真 despawn(补位)。
- [x] ✅ **抛射物(2026-07-02)**:`projectile` actuator 动作(`{speed,damage,radius,ttl,
  at:'player'|dir:[E,N,Alt],visual}`)——发射瞬间锁定方向,派生 Ball 实体 + `ProjectileComponent`,
  `ProjectileSystem` 直线飞行/命中/到期自毁;NPC 行为 enter 放 projectile + timeInState 循环 =
  **数据表达的炮塔**。
- [x] ✅ **命中判定(2026-07-02)**:球心距测试(vs 玩家胸高 + vs 活 NPC,射手免疫);
  **伤害仅 Game 模式落地**(飞行中切出模式=零伤害续飞)。
- [ ] ⏸️ **范围/DoT/击退/弹道重力/阵营**——按内容需求再议(spec §4 边界)。
- [x] 📄 `specs/combat-damage.md`(定稿+实现同日;含"接触伤害/炮塔/掉落"三配方)。
  测试 5(伤害/死亡+掉落/模式拒绝+无敌/玩家路由/飞行命中+TTL 过期)。

---

## F4 · 对话 / 任务 / 目标 / 进度 ✅ —— v1 已落地（2026-07-02，客户端 UI 待内容期）
...
- [x] ✅ **对话系统实现(2026-07-02)**:ba 行 slot 5 = dialogue 文档(`{start, nodes:{text,
  options:[{label, when?, actions?, to}]}}`);`DialogueSystem` 走树(interact.primary 启动、
  距离≤3.5m、JSONLogic 过滤选项可见性、actions 走 actuator、to 前进/null 结束);
  `world.activeDialogue` 单例 + `Engine.dialogueState/chooseDialogue/endDialogue` 门面;
  事件 dialogue.started/node/ended(载荷带 text+可见 labels——客户端 UI 纯渲染);
  **对话中 NPC 定身**(NPCSystem 跳过,结束恢复游荡,实测)。
- [x] ✅ **任务/目标 = flags 配方(有意不加原语,spec §3)**:接任务/推进/交付全由
  「对话选项 actions 设 flag + when 查 flags/inventory」表达(实测:带钥匙才见隐藏选项);
  等真实叙事内容暴露 flags 表达不了的结构再议(YAGNI 记录)。
- [x] ✅ **进度存档**:零新工作——flags 本就随 session meta 进 IDB。
- [ ] 🔲 客户端 UI:对话框(读 dialogue.* 事件 + 回调 chooseDialogue)/ 目标 HUD——
  内容期第一批客户端活(与 `PLAYABLE_CHECKLIST` G2 协同)。
- [x] 📄 `specs/dialogue-quests.md`(定稿+实现同日)。测试 3(启动+过滤+走树+结束动作/
  条件选项显隐/对话定身)。
...
3. ~~**F3（战斗）/ F4（对话任务）**~~ ✅ **v1 双双落地（2026-07-02）**——F 系列四项引擎原语全部就位;剩客户端对话 UI(内容期)+ 各项 v2 增强(寻路/阵营/长文档引用)。
