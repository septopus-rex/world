...
| 任务 | 落点 | 状态 |
|------|------|------|
| 合约 `AdjunctType` PDA（type_id u16 / manifest_cid / owner / version / status，约 103B/0.001 SOL） | `chain/` | ❌ |
| 全局 typeId 计数器 PDA（0x0100 起自增，社区类型分配） | `chain/` | ❌ |
| 合约指令 `register_adjunct_type(manifest_cid)` / `update_manifest` / `deprecate` | `chain/` | ❌ |
| 合约 `report_adjunct` 举报 + 状态机 | `chain/` | ❌ |
| **`AdjunctManifest` 格式标准**（TypeScript interface + JSON Schema，含 schema/impl_cid/source/github） | `engine/src/core/types/` | ❌ |
| 内建 adjunct Manifest 提取（0x0001-0x0008 硬编码，不走链上注册）| `engine/src/plugins/adjunct/manifests/` | ❌ |
| **移植沙箱**：旧引擎 `security/adjunct-{loader,sandbox}.js` → 新引擎 | `engine/src/core/services/` | 🟡 有原型 |
| `DynamicAdjunctLoader`：查链取 manifest_cid → 拉 Manifest → 按需拉 impl → 降级/沙箱 | 新引擎 | ❌ |
| `buildDefaultLogic(schema.render_hint)` 降级渲染 | 新引擎 | ❌ |
| `BlockSystem` 解析改为 name string key（对齐 IPFS JSON 格式） | 新引擎 | ❌ |
| `adjunctAPI` 受限接口（manifest.schema 约束驱动） | 沙箱 runner | ❌ |
| 资源由主线程经 StorageRouter 加载，**不进沙箱** | 新引擎 | ❌ |
| Fallback Box（橙色警告色，IPFS 不可用或 Banned 时） | 新引擎 | ❌ |

**验收**：发布 adjunct Manifest → 链上注册（得到 typeId）→ 世界数据引用该类型 → 引擎自动加载并渲染；IPFS 内容重传后 update_manifest 可恢复。
...
| **BTC OP_RETURN 写入 Genesis CID**（Launch 时机，一次性 ~$1–10） | ❌ |
| Genesis Manifest 永久存 Arweave（engine_cid + registry_cid）| ❌ |
| 灾难恢复文档：BTC → Arweave → 重建 Solana PDA 的操作步骤 | ❌ |
...
| D-RES | adjunct 代码与资源分开还是合并 | ✅ **存储统一、注册与治理分离**：代码→AdjunctType+沙箱+审计，资源→ResourceData+格式校验，代码引用资源 URI（§1.6）<br>**补充（Schema 优先）**：AdjunctType 进一步拆分为 schema_cid（必须，engine-agnostic）+ impl_cid（可选，engine-specific）；无 impl 时 render_hint 降级渲染，数据独立于引擎 |
| D-SIGN | 编辑发布的签名方式 | ✅ 编辑期改动本地缓存，发布时**批量序列化+上传+一次性/批量签名**（Phase 1） |
| D-ANCHOR | 系统级根信任锚 | ✅ **BTC OP_RETURN** 写入 Genesis CID（36B，一次 $1–10），Genesis CID 指向 **Arweave** 永久 Manifest（engine_cid + registry_cid）；Solana PDA 是活跃指针层，BTC 是不可变历史锚。灾难时从 BTC→Arweave→Solana 逐层重建。见 [§10](../architecture/onchain-storage.md#10-终极锚点btceth-不可变历史锚) |
