...
| **PR-1** 队列落地（共存）**——核心已落地（2026-06-12）** | ✅ `core/events/{EventTypes,EventChannel,EventReader,EventQueue,TargetKeys,index}.ts`；✅ World 加 `events/frame` 字段 + step 帧括号 + destroyEntity→dropTarget + dispose 清理；✅ 队列单测（`tests/unit/events/event-queue.test.ts`：双缓冲两帧生命期/游标落后告警/seq 排序/边界派发隔离/回调内 emit 次帧可见/dropTarget/scope/once/录制跳过 BOUNDARY_ONLY）。**未含**：`EventAliases.ts`、`core/services/EntityIndex.ts`、ISystem.init/dispose 透传（随 PR-2+ 接入时补） | `core/World.ts`、新目录、`tests/unit/events/` | 不动任何调用点，旧总线原样共存 |
| **PR-2a** interact 通道 **——已完成（2026-06-12）** | ✅ Raycast 三处 emit → `interact.primary/context/miss`（target/targetKey[adjunctId/blk:x_y]/actor）；✅ EditSystem/ItemDropSystem/**ItemSystem/TriggerSystem**（spec 后新增的两个消费者）全部改 reader；模式门控系统在被禁用时 `reader.clear()`；旧 emit 已删 | `core/systems/{RaycastInteraction,Edit,ItemDrop,Item,Trigger}System.ts` | 点选/触发/拾取 e2e 全绿 |
| **PR-2b** item/inventory 通道 **——已完成（2026-06-12）** | ✅ `item.pickup/consume/picked/dropped`（actor）→ InventorySystem reader；✅ InventorySystem 注册序移到全部发射者之后（同帧入包，注释固定意图）；✅ inventory.updated/full、item.spawn_drop、effect.spawn 换通道；Actuator bag 动作走 events | `core/World.ts`、`core/systems/{Inventory,ItemDrop,Item,ParticleEffect}System.ts`、`core/services/Actuator.ts` | pickup→inventory 同帧链路 vitest |
| **PR-2c** loader 通道 **——已完成（2026-06-12）** | ✅ `block.need`、`player.state`；✅ Engine 门面重铸（on/off/send、LEGACY_EVENT_MAP + 一次性 dev 告警、(payload, ev) 双参、按 (type,cb) 双键退订闭包、双总线订阅过渡）；✅ injectBlock 返回 EntityId；✅ DesktopLoader 改新名 | `Engine.ts`、`core/systems/GridSystem.ts`、`core/movement/CharacterController.ts`、`DesktopLoader.ts` | 块流式/持久化 e2e 全绿 |
| **PR-2d** 死线与 ui 通道 **——已完成（2026-06-12）** | ✅ `system.mode/system.preload`（world:save_request 删除）；✅ `edit.draft_saved/upload_request`；✅ EventUIProxy → `events.emit`（ui:* 名规范化为 ui.*，BOUNDARY_ONLY） | `core/World.ts`、`core/systems/EditSystem.ts`、`Engine.ts`（emitter 闭包） | 零订阅者，无行为回归 |
| **PR-3** block.loaded + boot gate **——已完成（2026-06-12）** | ✅ `BlockComponent.pendingAdjuncts/adjunctTotal` 计数 → 每块恰一次 `block.loaded`（target=块实体, targetKey=`blk:x_y`）；✅ DesktopLoader 启动门切 `{key, once}` 定向订阅 + 3s failsafe；✅ 旧 world:block_ready 直接删除（消费者唯一且同步切换，未走并行期） | `core/components/BlockComponent.ts`、`core/systems/{Block,Adjunct}System.ts`、`DesktopLoader.ts` | block.loaded 单发 vitest + 启动 e2e |
