...
## 10. 乘骑归属：一条轨道属于它所在的块（2026-08-20）

`CoasterSystem` 原本是**世界单例**：`buildPath` 扫全世界实体、取遇到的**第一个** coaster 主题 b6，
建好的 path **永不失效**。一个抄近路，掉出三个 bug：

| 症状 | 根因 |
|---|---|
| 世界里两条轨道，永远骑到其中一条，跟你在哪上车无关 | 取「第一个」，顺序由实体遍历决定 |
| 进任何 Game 区都被弹上轨道 | 只判 `mode===Game`，不问这是哪个块的局 |
| 骑完一次，之后每次进 Game 都重挂旧 path（其块可能早被驱逐） | path 跨会话缓存 |

第三条尤其黏：`GameZoneSystem` 在 `rideActive` 时冻结 zone 追踪（`game-mode-entry.md §5` 的乘骑例外），
被劫持之后**走不出去**。gallery 世界正好凑齐条件——它 include 了 coaster 关卡，走廊里又有一张
德州扑克桌（holdem，id 44）。

**修法：轨道 adjunct 就是声明。** 乘骑锚定 `world.activeGameBlock`——Pool / Mahjong / Shooting /
Tumble 共用的那个锚（Game 进入时由 `World.setMode` 冻结，所以跨块轨道不会把乘骑中途改指到别的块）：

- path 只从**该块上的** coaster b6 建；该块没有轨道 → 什么都不做（**这一行的缺席就是劫持本身**）。
- path **不跨会话缓存**——建一次只是几个 cell 的事，而这个 System 缓存过的 path 没有一次在下一局还是对的。
- 于是过山车与麻将桌同构，是**可放置的数据**：同一份块数据放两个坐标 = 两条各自能骑的轨道。

回归：`engine/tests/scenarios/coaster-ride.test.ts` 两项——「骑的是你上车那块的轨道」+
「没有轨道的 Game 区不碰玩家」。
