...
> 策略不同——因此拆成独立进程 `services/ai-builder/`（新端口 `7790`），可以单独测试、单独
...
- 独立进程 `services/ai-builder/`，端口 `7790`，`node:http` + `tsx`，无状态（世界快照随
...
| dev.sh | `deploy/dev.sh` | `FE_SERVICES` 新增一行 `AI-Builder|services/ai-builder|7790|npm start` |
