...
- 内置 id → 走 bundled pack（**离线默认，零网络依赖**）。**引擎只 ship `basic`（默认兜底）+ `coaster`
  （结构主题，带 `expandCell` 无法纯数据化）**；一切**视觉包是内容**（`client/desktop/src/stylepacks/*.json`）。
- CID/URL/id → 经 `IDataSource.stylePack(refs)` 解析 JSON → 校验 → `Engine.registerStylePack` →
  展开。**与 audio/video 的 `{raw, format}` 同款异步资源模型**（`DataSource.ts` 已有先例）。
- **数据分离已落地（②，2026-07-06）**：`brick`/`garden` 从引擎代码移到 client JSON；
  `stylepacks/index.ts` 是内容寻址 store（id + 内容 CID 都解析，stand-in IPFS/CAS）；
  `DesktopLoader.stylePack()` 实现 seam，boot 时 `registerStylePacksAtBoot()` 经 seam 预注册（local-first）。
  未解析的 theme（未注册 CID）→ **兜底 basic**（placeholder→swap）。**引擎端按块懒解析**（块 theme=CID →
  引擎遇到即 `dataSource.stylePack` 拉取注册）是"两头协调"的下一步。
...
- **B StylePack 外部化 + 风格可调** ✅ `Variants.ts`：`SppTheme` 加 `color`/`texture` 槽、
  ...
  **② 数据分离已落地（2026-07-06）**：brick/garden 出引擎代码 → `client/desktop/src/stylepacks/*.json`；
  `stylepacks/index.ts` 内容寻址 store（id + CID）；`DesktopLoader.stylePack()` 实现 seam +
  `registerStylePacksAtBoot()` 经 seam 预注册；引擎只留 `basic`/`coaster`，未解析 theme 兜底 basic。
  引擎端按块懒解析（block theme=CID → 懒拉）留作"两头协调"下一步。
