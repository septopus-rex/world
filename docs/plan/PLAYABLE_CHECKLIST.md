...
### G8 · 产品外壳 / 打磨 🔲
- [x] ✅ **默认入口的第一眼**（2026-08-17）—— gallery 走廊铺上环境层：铺装主路 + 缘石贯穿
      21 块、行道树成列、路灯/长椅/花盆按 4 块一循环。**改的不是展位，是展位之间的东西**。
      诊断见下方「走廊环境层」。
- [ ] 🔲 加载 / 错误 / 空状态
- [ ] 🔲 设置（音量/画质/灵敏度）
- [ ] ⏸️ 背景音乐 / 环境音
...
#### 走廊环境层（gallery，2026-08-17）

**诊断**：gallery 是 21 块的南北直线走廊（x=2000，y=1000..1020），一块一个展位，每块都铺着
同一张 16×16 草地板。所以展位是**孤立地立在无边草坪上**——没有路、没有边界、没有节奏。
「白模感」来自展位**之间**的空白，不是展位本身；先前几轮想靠调材质/加贴图去救展位，方向就错了。

**做法**（一次性脚本产出，按内容=数据纪律冻结进 `gallery.level.json`，脚本不入库）：

| 层 | 用什么 | 为什么不是别的 |
|---|---|---|
| 铺装 | a2 + `concrete-floor`(资源 47)，5m 车行道 + 两条 slate 缘石，贯穿每块 y 全长 | 逐块等长才能无缝接上；缘石是"这是一条路"最便宜的信号 |
| 行道树 | **a4 module 35 (`tree.gltf`)** | gallery 本来就在两块里种了这个模型。再摆一批球堆 prefab 树 = 一条路上两套视觉语言，**混着比单用任何一种都差**。prefab 树的位置是「没有模型资产的场景」，以及「第二引擎只凭数据复现」 |
| 路灯 / 长椅 / 花盆 | garden prefab，经 `expandPrefab` | 与世界 palette 点击盖章走同一个调用，产出的行就是作者手摆的行（§9.4） |

**三条别踩回去的**：

1. **必须逐块碰撞检查，固定模板一定穿模。** 展位是逐块布置的、没有公共网格——y=1002 的模型正好
   占着西侧行道树的位置。全量跑下来 **14 处候选位被让开**，不查就是十几棵树长在展品肚子里，
   而且要走完整条走廊才会发现。
2. **让开之后要找备选位，不能直接丢。** 第一版丢弃冲突项，y=1013 一棵树都没种上——**有洞的
   林荫道就不是林荫道**。改成沿路缘找最近空位。
3. **stop 要隐形**（`basic_stop.ts` 槽 6，同日新增）：不设的话每条长椅都坐在发光青箱子里。
