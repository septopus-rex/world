...
**现状（PR-1..3 已落地）**：核心通道运行在帧作用域类型化队列 `world.events` 上
——emit 只入队不跑回调，系统侧经 reader 拉取（同帧可见性由注册序决定），
边界回调（Engine 门面/UI/Loader）仅在 `step` 尾部 `flushBoundary` 按 `(frame,seq)`
全序派发，支持 EntityId 与稳定键（adjunctId / `blk:x_y`）双键定向。
触发器的 `touch` 即 `TriggerSystem` 对 `interact.primary` 的 reader 消费。
旧朴素总线（`World.on/emitSimple`）仍承载未迁移通道（`player:*` 生命周期、
`audio:played` 等），随 PR-4/5 退场——完整计划与各 PR 状态见
[事件总线设计规格](../plan/specs/event-bus-design.md) §7。
