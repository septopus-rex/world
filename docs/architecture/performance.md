...
*   **Block LOD**（`BlockLODSystem`，已实现）：距离超出 `world.performance.lodNear`
    （默认 40m）的地块，其 adjunct 网格组整体隐藏，地面板保留（远处仍读得出地形）；
    ...
    **距离按地块 AABB 的最近点算，不是块中心（2026-07-27 修正，别改回去）**：
    ...
    24m）。更要命的是它**各向异性**——流式窗口是方形，角块中心比正交块远 √2 倍
    （ext=2 时 45.3m vs 32m），于是 `lodNear=40` **恰好只裁掉四个角**，鸟瞰下
    窗口成了「井」字缺角。同一个 √2 也曾坑到雾（见下）。回归测试
    `systems/block-window-coverage.test.ts` + e2e `block-streaming.spec.ts` 的
    「田 not 井」用例。
*   **距离雾的半径按窗口对角算**（`EnvironmentSystem`，同日修正）：雾要盖住流式
    窗口之外的空白，但窗口最远的**内容**在对角线上。原先 `far = ext·块宽·1.2`
    （38.4m）短于角块中心（45.3m），四个角被整块刷成天空色——**加载了、仿真着、
    但看不见**。现为 `ext·hypot(块宽, 块长)`。教训：**方形窗口的半径不是边长的一半。**
