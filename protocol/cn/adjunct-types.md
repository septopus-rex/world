...
| `0x00a8` (168) | sign 标识牌 | 无光照贴图平面(路标/贴花/浮空导视) | §2.2 |
| `0x00b4` (180) | stop 碰撞体 | 隐形碰撞体,**三形状**:盒/圆柱/楔形坡 | §5 |
| `0x00b5` (181) | item 物品 | 可拾取物品(模板+seed 确定性实例) | §6 |
| `0x00b6` (182) | spp 弦粒子 | SPP 弦粒子源,展开为标准附属物 | §7 |
...
| `0x00e4` (228) | book 书 | 可点开翻页的文字面板(string[]) | §14 |
| `0x00e5` (229) | board 留言板 | 服务器频道留言墙(可读写的共享状态) | §14.1 |
...
| 3 | `resource` | number,缺省 `0` | **颜色**,双通道:`0` = 该型自身默认色 · `1..255` = 内建调色板索引(见 §2.3) · `≥256` = 字面 `0xRRGGBB`。**永不产生贴图** |
| 4 | `repeat` | `[u, v]`,缺省 `[1,1]` | 贴图平铺 |
| 5 | `animation` | 动画对象/`null` | Septopus 动画时间轴,见 [animation.md](animation.md) |
| 6 | `stop` | 任意/`null` | **truthy ⇒ solid**(`0`/`null`/`false`/空串=非 solid;a2 无视此槽恒 solid) |
...
### 2.2 a8 sign 标识牌(2026-07-12 增补)

无光照贴图平面——路标、海报、贴花、浮空导视箭头等**任何角度任何时刻都必须清晰可读**的图面。与 a2 贴图盒的三点刻意差异:无光照(不受昼夜/阴影影响)、单平面而非闭合盒(无"透印"镜像问题)、贴图整面贴合(0..1 UV,引用**不带 `size` 的贴图记录**,共享贴图 repeat 保持 [1,1])。无碰撞、不投影。
...
| 2 | 旋转(引擎系 Euler,同 §0) |
| 3 | 贴图资源 id |
| 4 | 透明度(可选,默认 1;`<1` 时贴图 alpha 生效——透明底"悬浮图形"用 0.95) |

**朝向契约**:rot=[0,0,0] 时平面**平躺**(法线朝上),贴图 V+(图像"上")指**北**——图里画一个朝上的箭头,世界里即指北。`rx` 为正让板面上缘向南倾(朝向北行者的仰角导视牌);`rx=π/2` 立正面南。


### 2.3 内建调色板(规范,2026-07-25 增补)

`resource` 槽的 `1..255` 段索引一张**引擎内建**的颜色 + PBR 表。跨引擎必须逐项一致——否则同一份地块数据在两个引擎里颜色不同。

| 索引 | 颜色 | rough / metal | 名 |
|---|---|---|---|
| 0 | 该型默认色(a2 = `#888888`) | 0.9 / 0 | 默认灰 |
| 1 | `#555555` | 0.85 / 0 | 深灰 |
| 2 | `#3d63a8` | 0.6 / 0 | 蓝 |
| 3 | `#c0392b` | 0.7 / 0 | 红 |
| 4 | `#b9b3a7` | 0.95 / 0 | 混凝土 |
| 5 | `#8f8578` | 0.95 / 0 | 旧混凝土 |
| 6 | `#9c5a3c` | 0.9 / 0 | 砖 |
| 7 | `#6f4b2f` | 0.8 / 0 | 木 |
| 8 | `#d8cbb0` | 0.9 / 0 | 砂岩 |
| 9 | `#3b3f46` | 0.7 / 0 | 板岩 |
| 10 | `#eeeeee` | 0.9 / 0 | 灰白(地面) |
| 11 | `#c8ccd0` | 0.35 / 0.9 | 钢 |
| 12 | `#b08d57` | 0.4 / 0.85 | 黄铜 |
| 13 | `#2a2d33` | 0.25 / 0.6 | 暗金属 |
| 14 | `#dcecf2` | 0.08 / 0.1 | 玻璃 |
| 15 | `#faf7f0` | 0.55 / 0 | 抹灰 |
| 16 | `#4a7c3f` | 0.95 / 0 | 草 |
| 17 | `#2f5233` | 0.95 / 0 | 深叶 |
| 18 | `#7a8b3f` | 0.95 / 0 | 浅叶 |
...
- **索引可改色,不可改义。** 2/3 号在 2026-07-25 从纯 `#3366ff`/`#ff0000` 调成颜料饱和度(索引不变、数据不动、既有场景一并受益);但把某个索引重新指给别的材质会静默改写所有引用它的地块。**只许追加。**
- **索引 0 = "未设"**,取该型自身默认色(a1 `#f8f8f8` · a5 `#44aaff` · a6/a7 `#cccccc` · a2 `#888888`),故绝大多数既有行(槽 3 = 0)不受调色板影响。
- **`rough`/`metal` 是渲染提示,不是几何**:无 PBR 概念的引擎可忽略,颜色仍须一致。
- a1/a5/a6/a7 的**槽 7 显式颜色优先于槽 3**(槽 7 存在时忽略槽 3)。a2 的槽 7 是贴图,故 a2 只能经槽 3 取色;贴图存在时颜色恒为白(由图供色),但 `rough`/`metal` 仍取自调色板项。
...
{ "initial": "idle",
  "states": {
    "<状态名>": {
      "move": { "kind": "stay"|"wander"|"follow"|"flee"|"return", "speed"?, "radius"?, "stopAt"? },   // speed 缺省 1 m/s(规范)
...
e 系媒体面板家族第 4 位(e1 链接 · e2 音频 · e3 视频 · **e4 书**):面板 +
资源 + 点击行为。点击(主交互射线)→ 宿主打开**翻页阅读器**(上一页 / 下一页 /
页码 N/M / 合上)。它是 ba NPC 对话树的**无生命孪生**——同为「台词」文字,但书是
**物件上的线性阅读器**,对话是**角色上的分支会话**;二者互不替代。

| 槽 | 字段 | 缺省 | 说明 |
|---|---|---|---|
| 0–2 | size/pos/rot | — | 同标准槽(默认竖立小书 `[0.7,0.2,0.9]`) |
| 3 | `resource` | `0` | 封面颜色/材质索引;有贴图则封面白化显真色 |
| 4 | `repeat` | `[1,1]` | 贴图平铺 |
| 5 | `animation` | `null` | Septopus 动画 |
| 6 | `stop` | `null` | 非空 ⇒ solid |
| 7 | `pages` | `[]` | **书页**:内联 `string[]`(开发期明文)**或** 解析为 `string[]` 的资源 id/**IPFS CID**(生产;大段文字不入地块行,同 e2/e3 `source`)。空书为惰性(不报错) |
| 8 | `title` | `''` | 阅读器标题栏文字,**并烫印在封面书名牌上**(见下) |

翻页是**纯视图动作**(页码状态属客户端,同 e1 的 `window.open` 留在客户端的纪律);
引擎只渲染书体 + 承载文字,点击发通用 `interact.primary`。页码钳制在 `[0, M-1]`
(不回环)。

**书名的位置(2026-07-29 修订)**:`title` 非空时,渲染层把它排版进**两面封面的书名牌**
(牌子随之放大以容纳文字);为空则退回一块无字的小铭牌。书名**不再浮在书体上方**——
浮空标签留给 e5/e1(墙挂面板没有封面可印)。字号由牌子尺寸与字串长度解出,故同一本书
在不同 `size` 下的观感一致;换行**优先在空白处**,词内断行只在单个词宽于整行时发生。
...
[[0.7,0.2,0.9], [11,8,1.2], [0,0,0], 0, [1,1], null, null,
 ["第一页:点击书本可以翻页。",
  "第二页:文字以 string[] 承载,可存于 IPFS。",
  "(合上书本。)"],
 "无题之书"]
```

### 14.1 e5 board 留言板(2026-07-08 增补)

```
[ size, pos, rot, resource, repeat, animation, stop, channel, title? ]
```

标准 7 槽 + slot 7 `channel`(字符串,缺省 `'lobby'`)+ 可选 slot 8 `title`。
e 系面板家族第 5 员;与 e4 book 的关键区别:**book 的内容在数据里(不可变),
board 的内容是服务器上的可变共享状态**——数据只声明频道 id,读写由宿主经
留言服务完成(离线降级只读),与 Game 模式「块声明意图、宿主拨服务」同构
([game.md](game.md) §2/§9:会话状态不上链)。多块可共享一个 channel。
点击经 `interact.primary`;呈现(面板 UI)属宿主层,非规范。

## 15. 动态类型区段(规范,2026-07-08)

- **内置区段 `0x0000–0xefff` 保留给本协议**:内置类型的语义由本文规定、各引擎原生实现,
  **不可**被数据(动态模块)声明或覆写——允许覆写即允许重定义"墙"或抢注未来内置槽位。
- **动态区段 `0xf000–0xffff`**:经 `septopus.adjunct.module` 文档
  (封套与加载见 [envelope.md](envelope.md))注册的自定义类型**只准**落在此区段;
  越界注册 = 实现必须拒绝。同一世界内同 typeId 的两个模块 = 加载错误(快败)。
- 动态类型的跨引擎真相是模块的 **descriptor**(预求值声明式产物);`code` 是可选的
  生成器路径(能跑沙箱的引擎可用,并须验证产物与 descriptor 一致)。

---

*协议版本:v0.1(对应引擎 v0.1.0;2026-07-08 增补 §15)。变更须同步 cn/en 双语并记入根 CHANGELOG。*
