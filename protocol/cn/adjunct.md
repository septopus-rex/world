...
- **颜色**：全在 **raw 槽 3**（一个槽两通道）：`0` = 该型默认色 · `1..255` = **内建调色板索引（现为规范）** · `≥256` = 字面 `0xRRGGBB`。逐项颜色表与规矩见 [adjunct-types.md](./adjunct-types.md) §2.3。（2026-07-25 前调色板只有 4 项、且标为「非规范」，a1/a5/a6/a7 更是整槽被丢弃；现已成表并全族生效。a1/a5/a6/a7 的槽 7 显式色仍优先；a2 的槽 7 是贴图。）
