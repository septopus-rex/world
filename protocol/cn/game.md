...
## 9. 游戏会话与验证 (Game Session & Verification)

> 目标：在 **local-first、链可选**的架构下回答「成就/奖励如何取信」——例如
> "桌球玩满 100 次得一枚奖章"。核心结论先行：**裸 hash 不能防伪**（hash 人人可算，
> 只证完整性不证真实性）；真实性 = **签名（hash + 玩家拿不到的私钥）**，
> 且签名的前提是存在一个**玩家写不进去的计数器**（服务器权威）。

### 9.1 信任边界（何时才需要防伪）

```
纯本地（单机）        →  不设防。自己的存档骗自己 = 单机改存档，local-first 本意如此。
展示 / 交易 / 多人 / 上链 →  跨信任边界，才需要证明。
```

### 9.2 服务器 = 同一协议的另一个解释器

服务器是**不带渲染器的 VM**（本引擎可 headless 运行——测试套件即证明，校验服务器可
直接复用引擎做重放）。"遵循协议"有两层：

1. **规则层**：验证/重放用的规则与客户端逐位一致（物理步进、发牌推导、判定）；
2. **数据层**：呈现结果读同一套数据词汇（物品 `{templateId, seed}`、收据、Septopus 坐标）。

### 9.3 会话的规范形：源 + 重放（Normative）

**一局游戏 = `(seed, 操作序列)`** —— 这是唯一要存的「源」；局面与结果是
**确定性重放的推导物，不持久化**（与 item.md「可推导之物永不持久化」同一铁律）。
引擎的确定性（`step(dt)`、全 seeded PRNG、无 `Math.random`/墙钟）是重放可行的地基。

```
一局游戏  = (seed, 操作序列)              ← 唯一要存的源
局面/结果 = 确定性重放(源)                ← 推导物，不存
公平性    = commitment（局前锁 seed，§9.5） ← hash 的正确用途
真实性    = 服务器签名收据（§9.4）          ← 跨信任边界的凭证
```

### 9.4 成就凭证：签名收据（Receipt）

服务器权威计数达成（如第 100 局上报）后签发：

```
receipt = sign(server_key, { player, achievement, seed, issued_at })
```

- 奖章物品照旧是 `{templateId, seed}`（item.md），**收据放 `metadata`**；
- 本地背包仍可伪造物品——但验证方持服务器公钥验签即揭穿假奖章：
  **信任锚从「数据本身」移到「签发者的钥匙」**；
- 升级路径连续：接链后同一张收据锚上链即可。

### 9.5 按游戏类型的会话形态

关键差异是**有没有隐藏信息**：

| 游戏 | 随机源 | 隐藏信息 | 最小存储（可验证） | 托管模式 |
|---|---|---|---|---|
| 迷宫 | 无（固定 seed 雕刻，authored 内容） | 无 | 仅**操作序列** | C/B |
| 桌球 | 无 / 固定摆位 | 无 | **seed + 击球序列** | B + 事后重放验证 |
| 棋牌（麻将） | **发牌** | **有**（手牌/牌墙保密） | **发牌 seed（+承诺）+ 操作序列** | **A**（服务端权威） |

**棋牌规范**：发牌**不存快照，存种子**——牌墙 = `shuffle(seededRng(seed))` 的纯函数
（与物品 seed 推导同一 DNA）。公平性用**承诺-揭示（commit-reveal）**闭环：

```
局前：服务器公布 commitment = hash(seed ‖ salt)
局中：按 seed 推导的牌墙发牌；操作序列记录在案
局后：揭示 seed + salt → 任何人验证：
      ① hash(seed‖salt) == commitment    （发牌局前已定，未临时换牌）
      ② 重放(seed + 操作序列) == 宣称结果
```

隐藏信息决定托管：局中牌墙只有服务器知道（否则看穿对手手牌）——这是选型口诀
「需服务端权威才 A」的具体化；无隐藏信息的（迷宫/桌球）可 Pattern B 本地跑 + 抽查重放。

### 9.6 现状与缺口

| 已有 | 缺 |
|---|---|
| `IGameApi` + methods 白名单 + `FetchGameApi`（服务器通道，麻将 `?mjserver` 已示范） | Pattern B 游戏（桌球/叠叠乐）无**局终结果上报 seam**（各 System 局终只改本地状态） |
| 物品 `{templateId, seed}` + `metadata`（收据有处安放） | 收据签发/验证的具体字段与密钥分发约定 |
| 引擎确定性（重放验证地基） | 输入序列录制器（做重放验证时再补，YAGNI） |
