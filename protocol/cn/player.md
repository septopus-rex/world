# SPP 玩家协议 (Player Protocol)

**String Particle Protocol (SPP)** 并不只是将“玩家 (Player)”概念化为一个漂浮的相机视角，而是将其视为世界中的一个物理实体，其状态、移动和视觉表示（化身 Avatar）都必须遵守严格的标准。

## 1. 玩家空间状态 (Player Spatial State)

与静态地块不同，玩家的状态是高度动态的。SPP 引擎必须持续追踪玩家的世界定位和姿态，以计算物理效果和渲染边界。
