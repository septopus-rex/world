...
- **air 需迟滞（规范要求,coyote time）**：许多角色控制实现中 `isGrounded` 在**平地上
  ...
  `_airborneSec`,落地清零。
- **原地转身（turn-in-place，表现层）**：站立中（`hSpeed ≤ IDLE_MAX`）且视角输入**按住
  未松**（鼠标拖拽/触屏拖动/视角键/手柄右摇杆）时，角色的**视觉朝向冻结不跟**——可以
  绕到角色正面；输入**松开后**视觉朝向以受限角速度追赶权威朝向（参考值 ≈ 300°/s，最短
  弧），追赶期间（剩余角差 > 约 7°）播放 `walk` 剪辑作原地碎步。移动中不冻结（移动是
  相机相对的，身体必须摆向行进方向，仍受限速）。**不得只做逐帧限速而不冻结**：正常拖拽
  每帧目标角差远小于限速步长，角色会逐帧贴住相机、碎步永不触发（已实测，2026-07-22）。
  这是速度→状态派生的**唯一例外**，且只作用于视觉层：**权威 transform 朝向（移动方向
