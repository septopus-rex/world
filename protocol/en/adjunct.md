...
- **Primitive semantics**: `box(w,h,d)` centered full-extent; `sphere` radius = `w/2`; `cylinder/cone(w,h,d)`; `plane(w,h)`; `tube` = Catmull-Rom extrusion along control points; `wedge(w,h,d)` = ramp rising from 0 to `h` along one horizontal axis. **Segment counts (e.g. sphere 32×32) are pixel-level detail — engines may differ** (behavior-equivalent).
- **`wedge` is the visual twin of the `b4` slope collision shape**: its inclined face MUST be solved from the **same plane equation** as the collision side's `topYAt`, or the player floats above / sinks into the ramp (see [adjunct-types.md](./adjunct-types.md) §5).
...
- **Color** lives entirely in **raw slot 3** (one slot, two channels): `0` = the type's default colour · `1..255` = the **built-in palette index (now normative)** · `>=256` = a literal `0xRRGGBB`. The entry-by-entry table and its rules are in [adjunct-types.md](./adjunct-types.md) §2.3. (Before 2026-07-25 the palette had four entries and was marked non-normative, and a1/a5/a6/a7 discarded the slot entirely; it is now a real table honoured by the whole family. An explicit slot-7 colour still outranks it on a1/a5/a6/a7; a2's slot 7 is the texture.)
