...
| | | `enterGame` / `exitGame` | `[{ exitPolicy?, lockMovement? }]` | Data-driven Game-mode entry/exit (**zone-gated**: only succeeds inside a block with `block.game≥1`); `exitPolicy` = `ephemeral` (walk off the block → teardown, default) / `confirm` (leave dialog) / `persistent` (save & resume, planned). `lockMovement` (default `false`): while set, the player cannot walk/jump for the session (camera look still free) — for seated gameplay (card/board tables); games that don't declare it are unaffected (pool/mahjong players can still walk around the table). Unconditionally cleared on leaving Game mode however the session ends (including an abnormal exit), so the player can never get stuck. |
