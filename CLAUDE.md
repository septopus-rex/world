...
- **三种游戏托管模式**：A 外部 app（GameRuntime+HUD，逻辑可服务端跑）· B 原生 System（Pool/Mahjong/ShootingRange/Tumble，细节见 `engine/CLAUDE.md`）· C 纯数据驱动（authored 块数据 + 通用 Trigger/Actuator/Flag/Health，零专用代码，跑酷/coaster = `client/core/src/levels/*.level.json`）。**选型口诀：先 C，不行再 B，需服务端权威才 A。**
