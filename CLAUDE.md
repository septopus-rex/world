...
```bash
# 客户端 —— 用 npm(desktop 7777 · mobile 7778 · editor 7779,共享 client/core)
...
- **三种游戏托管模式**：A 外部 app（GameRuntime+HUD，逻辑可服务端跑）· B 原生 System（Pool/Mahjong/ShootingRange/Tumble，细节见 `engine/CLAUDE.md`）· C 纯数据驱动（authored 块数据 + 通用 Trigger/Actuator/Flag/Health，零专用代码，**跑酷** = `client/core/src/levels/parkour.level.json`）。**过山车不是 C**：轨道几何/连通确实是 SPP 数据，但载客要专用的 `CoasterSystem`（= B，`GAME_SYSTEMS_BACKLOG.md` C3 早标了「Pattern-B 味」）——按 C 去理解它，就会以为「再放一条轨道只要写 JSON」。**选型口诀：先 C，不行再 B，需服务端权威才 A。**
  **同一款游戏出现在两种模式里时，规则必须抽成两边共用的纯函数核**——麻将是先例（`engine/src/core/mahjong/`，见 `engine/CLAUDE.md`）：3D 桌（B）与外部 app（A）曾各写一半、互相矛盾。判据是「这段逻辑换个引擎/换个宿主还成立吗」——成立就不属于 System。
...
- 真 WebGL / 像素 / 输入（L4）用 Playwright，在 `client/desktop/e2e/`（现查 `ls client/desktop/e2e/*.spec.ts | wc -l`，**60+ 个 spec**：boot/movement/fall-through/trigger/avatar/persistence/inventory/engine-features/editor-platform/spp/coaster/map2d/game-trigger/ai-authoring/rpg-xianjian/portal-travel/worldlabs-panel/palace/book-cover-title/world-clock 等；**`world-clock` 是唯一不调 `engine.stop()` 的 spec**——它验的正是客户端自己的 rAF 在推进世界时钟，其余 spec 全部手动 `step()`，一个 rAF 死掉的世界能让它们全绿；`npm run test:e2e`，SwiftShader 软渲染 + `engine.step(dt)` 确定性驱动）。
