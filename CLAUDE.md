...
- `docs/plan/specs/combat-damage.md` + `dialogue-quests.md` — **F3 战斗 / F4 对话 v1(已实现 2026-07-02)**:F3 = `damage`/`projectile` actuator 动作(仅 Game 模式)+ ba 行 hp 槽 + NPC 死亡(onDeath 动作=掉落 spawn b5;authored 死体隐藏保留防 draft 丢行,派生真 despawn)+ `ProjectileSystem`(直线/球测/TTL);F4 = ba 行 dialogue 槽(nodes/options/JSONLogic when/actuator actions/to)+ `DialogueSystem` 走树(interact 启动、单例、NPC 对话定身)+ `Engine.dialogueState/chooseDialogue/endDialogue`;任务=flags 配方(有意不加原语);客户端对话 UI 待内容期。
