...
- **昼夜与天气闪电**：动态日光已恢复（2026-07-03，`EnvironmentSystem.DAYLIGHT`：晨昏带 smoothstep 渐变替代 isDay 二值跳变 + 视觉太阳角/强度按 chase 速率追赶跳变；**阴影仍关**，待 shadow bias 调参，见 RenderEngine 注释）；雷雨（rain + grade≥1）按 grade 缩放的确定性定时器触发全场闪光（环境光/太阳强度尖峰，`updateLightning`）。**日内时钟与链历法拆分（2026-07-13）**：太阳时/分/秒是本地按 dt 连续推进的日内时钟（不依赖链，见上文"天气/历法确定性派生"），链高度只跳"日历日期"这个整数，不再跳太阳位置——chase 现在主要是兜底而非主力（日内时钟本身逐帧平滑推进，本就不产生跳变）。
