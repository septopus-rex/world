...
- **原生在场游戏（Pattern B）**：`PoolSystem`/`MahjongSystem`/`ShootingRangeSystem`——System 持逻辑、棋子即 adjunct 实体、点击经 `interact.primary` 触发动作（规格 `docs/plan/specs/native-in-world-games.md`，记忆 `native-in-world-game-pattern.md`）。**生命周期区域门控**（复用上条 Game 契约）：块标 `block.game=1`（纯可玩标记，非外部 app id 42/43），`configure` 只**登记(arm)**，每帧 `syncSession` 按「`mode===Game` 且玩家在本块」spawn/teardown——**走出 block（1 格）即自动退回 Normal 拆局**（比 5×5 块驱逐更早，根除悬空实体），armed config 跨驱逐保留→重入即新局（街机柜模型，无中途存档）。**⚠️ 设计校正（文档已改、代码待跟进，见 `game-mode-entry.md`）**：当前是「声明写 block + 走出即拆」的 `ephemeral` 单块版；目标把游戏富声明迁到 **game trigger**（`enterGame` 带 `gameId`/`origin`/`exitPolicy`，一块可多台＝一排扭蛋机），block 降为粗粒度"此处可玩"位，退出做成 per-game **`exitPolicy` 三档**（`ephemeral`/`confirm` 弹框/`persistent` 存档重入），**不新增第二个 SystemMode**。**运行时改色**经 `core/utils/Appearance.ts setEntityColor`（写 `MeshComponent.colorOverride`→`VisualSyncSystem`→`RenderEngine.updateObjectAppearance` clone-on-write 隔离材质，不染共享材质邻居）。**三种托管模式**：A 外部 app（GameRuntime+HUD，逻辑可服务端跑，id 42/43）·B 原生 System（本条）·C 纯数据驱动（authored 块数据 + 通用 Trigger/Actuator/Flag/Health，零专用代码，**跑酷=`core/levels/parkour.ts`**）；选型口诀**先 C 不行再 B、需服务端权威才 A**。
