...
- `docs/plan/specs/palace-stress-level.md` — **宫殿关卡(规划 2026-07-18,未实现)**:6×6 连片宫殿于 [2100..2105]×[1100..1105](`?level=palace`)——三环结构(外圈 20 功能房+回廊 12+内院 2×2),一石二鸟:二维连片建筑的**流式压力测试**(常驻窗口 5×5 硬定界、窗口外立即驱逐——压驱逐正确性/重进入确定性/块边界/会话状态/帧预算,**不改 extend**)+ **全 adjunct 收纳架**(21 内置覆盖 20,棋牌四房 ref 冻结块、东翼跨房间 flags 任务链、b6 terran 客房、大门/水池刻意骑缝作接缝试金石);块切割纪律=房间边界压块边界+北墙西墙自有;产出=一次性生成脚本冻结 level JSON(游戏房构建期合并外壳);验证=headless scenario(驱逐/确定性/资源归零断言)+e2e 正确性+真浏览器 renderer.info 基线(灯光数=第一预期瓶颈)。
